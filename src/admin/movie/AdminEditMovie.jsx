...
const AdminEditMovie = () => {
  ...
  const { movieId } = useParams(); // Lấy movieId từ URL nếu cần
  ...
  // Fetch data from API to edit a movie (GET request)
  useEffect(() => {
    const fetchMovieData = async () => {
      try {
        ...
        // Set the movie data into the state
        ...
        setMovie({
          ...
          lang: movieData.Lang,
          year: movieData.Year,
          actorIds: movieData.Actors.map(x => x.Id) || [],
          countryIds: movieData.Countries.map(x => x.Id) || [],
          directorIds: movieData.Directors.map(x => x.Id) || [],
          categoryIds: movieData.Categories.map(x => x.Id) || [],
        });
        setEpisodes(movieData.StreamData.flatMap(server => 
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log("Updated movie values:", movie);
    ...
    Object.entries(groupedEpisodes).forEach(([serverId, episodes]) => {
      ...
      episodes.forEach((episode, episodeIndex) => {
        formData.append(
          `StreamData[${streamDataIndex}][Episodes][${episodeIndex}][Id]`,
          episode.Id
        );
        formData.append(
          `StreamData[${streamDataIndex}][Episodes][${episodeIndex}][Name]`,
          episode.Name
        );
        formData.append(
          `StreamData[${streamDataIndex}][Episodes][${episodeIndex}][LinkEmbed]`,
          episode.LinkEmbed
        );
        formData.append(
          `StreamData[${streamDataIndex}][Episodes][${episodeIndex}][LinkM3U8]`,
          episode.LinkM3u8
        );
      });
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <h1 className="text-2xl  text-slate-800 font-bold mb-4">Edit Movie</h1>
      <form onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <Select
            ...
            name="actorIds"
            options={actors}
            value={actors.filter((actor) =>
              movie.actorIds.includes(actor.value)
            )}
            onChange={(selectedOptions) =>
              handleSelectChange(selectedOptions, "actorIds")
            }
            className="w-full p-2 border text-gray-700 border-gray-300 rounded-lg focus:ring focus:ring-blue-300 focus:outline-none"
        ...
        <div>
          ...
          <Select
            ...
            name="categoryIds"
            options={categories}
            value={categories.filter((actor) =>
              movie.categoryIds.includes(actor.value)
            )}
            onChange={(selectedOptions) =>
              handleSelectChange(selectedOptions, "categoryIds")
            }
            className="w-full p-2 border text-gray-700 border-gray-300 rounded-lg focus:ring focus:ring-blue-300 focus:outline-none"
